...
CleanSolve Studio는 수학 손글씨 풀이 정리 에디터입니다. 원본 문제 이미지를 변경하지 않고, 선생님 손글씨 풀이를 후보 렌더링 spec으로 추출한 뒤 검증, deterministic overlay 렌더링, 자동 자기수정, 필요한 항목만 사람 검토로 넘기는 흐름을 목표로 합니다.

## 현재 포함된 것

- Pydantic 기반 후보 spec 모델과 검증 규칙
- 치수 표시 중심 deterministic SVG overlay renderer
